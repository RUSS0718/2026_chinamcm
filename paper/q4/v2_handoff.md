# Q4 V2 论文手交接（REVIEWING）

状态：`REVIEWING`。这是方法与阶段性证据交接，不是 `VERIFIED`/`FINAL`，不应直接作为最终论文结论提交。
...
- 不能把本轮 Agent `REVIEWING` 状态写成 `VERIFIED`/`FINAL`，不能把材料迁入 `outputs/q4/final/`。
- 不能把本轮结果外推到题面未确认的几何厚度、其他旋转许可、连续参数或 Q1--Q3 模型。

## V3 待办

1. 由人工复核人确认几何来源、b1 厚度 2、协议和 exact 声明域，再决定是否提升阶段状态。
2. 若需要扩展，增加连续域/几何敏感性和预算敏感性实验，并保持整数 exact 与 SA 的同一正式评价/独立 audit 口径。
3. 完成全量回归、论文图文审校和最终结果门槛后，才讨论 `VERIFIED`/`FINAL` 与正式发布目录。
